...
@pytest.mark.parametrize(
    "test_input,expected",
    [
        ("define test = 42 i32", SymbolType.Constant),
        ("define test = binding", SymbolType.Unknown),
    ],
)
def test_definition_rule(parser, test_input, expected):
    ...
    assert expected == symbol_table_pass.symbol_table["test"]


def test_definition_rule_error_already_defined(parser):
    tree = parser("definition").parse("define test = 42 i32")
    symbol_table_pass = ToSymbolTable()
    symbol_table_pass.symbol_table["test"] = SymbolType.Function
    result = symbol_table_pass.transform(tree)
...
def test_struct_rule(parser):
    ...
    assert "test" in symbol_table_pass.symbol_table
    assert SymbolType.Struct == symbol_table_pass.symbol_table["test"]
...
def test_function_rule(parser):
    ...
    assert "test" in symbol_table_pass.symbol_table
    assert SymbolType.Function == symbol_table_pass.symbol_table["test"]
...
def test_function_rule_error_already_defined(parser):
    ...
    symbol_table_pass = ToSymbolTable()
    symbol_table_pass.symbol_table["test"] = SymbolType.Struct
    result = symbol_table_pass.transform(tree)
...
def test_label_rule(parser):
    ...
    assert "test" in symbol_table_pass.symbol_table
    assert SymbolType.Label == symbol_table_pass.symbol_table["test"]
